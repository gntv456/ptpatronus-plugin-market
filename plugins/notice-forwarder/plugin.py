...
def handle_event(event_type, data, cfg):
    ...
    body="事件已触发："+str(event_type)
    if as_bool(cfg.get("include_payload"), False) and data: body += "\n"+json.dumps(data, ensure_ascii=False)[:2000]
    notice(f"{str(cfg.get('title_prefix') or '[插件]').strip()} {event_type}", body, level=str(cfg.get("level") or "info"))
def handle_action(action, inp, cfg, host):
    if action!="send_test_notice": raise ValueError("unknown action")
    title=str(inp.get("title") or "插件测试通知").strip(); body=str(inp.get("body") or "事件通知桥工作正常。").strip()
    notice(title, body, "info"); return {"sent":True,"title":title}
