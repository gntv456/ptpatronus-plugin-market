...
def handle_event(event_type, data, cfg):
    return
def _send_line(line, timeout=20):
    parts=[p.strip() for p in line.split("|")]
    if len(parts)<4: raise ValueError("格式 name|post_url|cookie|message")
    name, url, cookie, message = parts[0], parts[1], parts[2], "|".join(parts[3:])
    data=urllib.parse.urlencode({"message": message, "content": message, "shout": message}).encode()
    req=urllib.request.Request(url, data=data, headers={"User-Agent":"PTPatronus-GroupChat/1.0","Cookie":cookie,"Content-Type":"application/x-www-form-urlencoded"}, method="POST")
    with urllib.request.urlopen(req, timeout=timeout) as res:
        body=res.read(500).decode("utf-8","replace"); code=getattr(res,"status",200)
    return {"name":name,"ok": code<400, "status":code, "snippet":body[:160]}
def _run_block(raw):
    results=[]
    for line in str(raw or "").splitlines():
        line=line.strip()
        if not line or line.startswith("#"): continue
        try: results.append(_send_line(line))
        except Exception as e: results.append({"ok":False,"error":str(e),"line":line[:80]})
    return results
def handle_action(action, inp, cfg, host):
    if action=="preview":
        return {"main":split_multi(cfg.get("targets")), "zm":split_multi(cfg.get("zm_targets"))}
    if action=="run_main":
        results=_run_block(cfg.get("targets"))
    elif action=="run_zm":
        results=_run_block(cfg.get("zm_targets"))
    else:
        raise ValueError("unknown action")
    ok=sum(1 for r in results if r.get("ok"))
    if as_bool(cfg.get("notify"), True): notice("群聊区", f"{action} 成功 {ok}/{len(results)}", "info" if ok==len(results) else "warning")
