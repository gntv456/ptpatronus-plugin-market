...
def _match(data, cfg):
    kws=split_multi(cfg.get("keywords")); fields=split_multi(cfg.get("fields")); cs=as_bool(cfg.get("case_sensitive"), False)
    hits=[]; field_hits=[]
    def check(field, text):
        t=text if cs else text.lower()
        for kw in kws:
            cmp=kw if cs else kw.lower()
            if cmp and cmp in t:
                if kw not in hits: hits.append(kw)
                if field not in field_hits: field_hits.append(field)
    if not fields:
        check("$payload", json.dumps(data, ensure_ascii=False))
    else:
        for f in fields:
            cur=data
            ok=True
            ...
            if ok and cur is not None: check(f, str(cur))
    return {"matched": bool(hits), "keywords": hits, "fields": field_hits}
def handle_event(event_type, data, cfg):
    allowed=split_multi(cfg.get("event_types"))
    if allowed and event_type not in allowed: return
    res=_match(data if isinstance(data,dict) else {"data":data}, cfg)
    if res["matched"]:
        notice(f"关键词告警: {event_type}", f"命中 {', '.join(res['keywords'])}", level=str(cfg.get("level") or "warning"))
        log("warning", event_type, "keyword alert matched", res)
def handle_action(action, inp, cfg, host):
    if action!="test_match": raise ValueError("unknown action")
    sample=inp.get("payload") or inp
    if isinstance(sample,str):
        try: sample=json.loads(sample)
        except Exception: sample={"text":sample}
    return _match(sample, cfg)
