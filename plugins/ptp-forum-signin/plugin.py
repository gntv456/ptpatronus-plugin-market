...
def handle_action(action, inp, cfg, host):
    if action=="labels":
        return {"sites":["generic-url","nexusphp-attendance"]}
    if action not in ("run","run_now"): raise ValueError("unknown action")
    results=[]
    for line in str(cfg.get("sites") or "").splitlines():
        line=line.strip()
        ...
        headers={"User-Agent":"PTPatronus-ForumSignin/1.0"}
        if cookie: headers["Cookie"]=cookie
        try:
            req=urllib.request.Request(url, headers=headers)
            with urllib.request.urlopen(req, timeout=20) as res:
                body=res.read(2000).decode("utf-8","replace"); code=getattr(res,"status",200)
            ok = code < 400
            results.append({"name":name,"ok":ok,"status":code,"snippet":body[:200]})
        except Exception as e:
            results.append({"name":name,"ok":False,"error":str(e)})
    ok_n=sum(1 for r in results if r.get("ok"))
    if as_bool(cfg.get("notify"), True):
        notice("论坛签到", f"成功 {ok_n}/{len(results)}", "info" if ok_n==len(results) else "warning")
    kv_set("last_signin", json.dumps(results, ensure_ascii=False)[:3000])
