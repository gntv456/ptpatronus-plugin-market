...
def handle_event(event_type, data, cfg):
    payload = data if as_bool(cfg.get("include_payload"), True) else {}
    log("info", event_type or "event", "audit event received", payload if isinstance(payload, dict) else {"data": payload})
def handle_action(action, inp, cfg, host):
    if action=="echo": return {"input": inp, "at": datetime.now(timezone.utc).isoformat()}
    if action=="emit_test_event":
        data={"source":"event-audit","message":str(inp.get("message") or ""),"at":datetime.now(timezone.utc).isoformat()}
        publish_event("plugin.test", data); return {"published":True,"event_type":"plugin.test","data":data}
