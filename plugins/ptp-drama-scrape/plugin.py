...
_status={"running":False,"done":True,"ok":0,"miss":0,"errors":0}
_lock=threading.Lock()
def handle_event(event_type, data, cfg):
    return
def _search(base, key, name, timeout=15):
    q=urllib.parse.urlencode({"keyword": name, "key": key} if key else {"keyword": name})
    url=base.rstrip("/") + "/api/search?" + q
    try:
    ...
    for it in items[:10]:
        ...
        out.append({"id":it.get("id"),"title":it.get("title") or it.get("name"),"year":it.get("year") or it.get("releaseYear"),"cover":it.get("cover") or it.get("poster"),"raw":it})
    return out
def _worker(cfg):
    global _status
    ok=miss=err=0
    roots=split_multi(cfg.get("scan_paths"))
    base=str(cfg.get("cunguangji_base") or "https://api.cunguangji.com")
    key=str(cfg.get("cunguangji_key") or "")
    try:
        for root in roots:
            p=Path(root)
            ...
            for d in p.iterdir():
                ...
                name=d.name
                try:
                    results=_search(base, key, name)
                    if not results:
                        miss+=1; continue
                    meta=results[0]
                    if as_bool(cfg.get("write_nfo"), True):
                        nfo = d / (name + ".nfo")
                    ...
                    err+=1
        notice("短剧刮削完成", f"成功 {ok}, 未命中 {miss}, 错误 {err}", "info" if err==0 else "warning")
    finally:
        with _lock:
            _status.update({"running":False,"done":True,"ok":ok,"miss":miss,"errors":err,"finished_at":datetime.now().isoformat(timespec="seconds")})
def handle_action(action, inp, cfg, host):
    global _status
    if action=="scrape_one":
        name=str(inp.get("name") or "").strip()
        if not name: raise ValueError("input.name 必填")
        base=str(cfg.get("cunguangji_base") or "https://api.cunguangji.com")
        return {"name":name, "results": _search(base, str(cfg.get("cunguangji_key") or ""), name)}
    if action=="status":
        with _lock: return dict(_status)
    if action=="scan":
        with _lock:
            if _status.get("running"): return {"started":False,"already_running":True,"status":dict(_status)}
            _status={"running":True,"done":False,"ok":0,"miss":0,"errors":0,"started_at":datetime.now().isoformat(timespec="seconds")}
        threading.Thread(target=_worker, args=(cfg,), daemon=True).start()
        return {"started":True,"status":dict(_status)}
