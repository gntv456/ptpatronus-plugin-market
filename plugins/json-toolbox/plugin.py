...
def _get_path(obj, path):
    ...
    for part in str(path or "").split("."):
        ...
        if isinstance(cur, list):
            ...
            raise ValueError(f"无法在 {type(cur)} 上取 {part}")
    return cur
def handle_action(action, inp, cfg, host):
    raw=inp.get("json") if inp.get("json") is not None else inp.get("text")
    if action=="validate":
        try: _parse(raw); return {"valid":True}
        except Exception as e: return {"valid":False,"error":str(e)}
    obj=_parse(raw)
    if action=="format":
        return {"json": json.dumps(obj, ensure_ascii=False, indent=as_int(cfg.get("indent"),2) or 2)}
    if action=="minify":
        return {"json": json.dumps(obj, ensure_ascii=False, separators=(",",":"))}
    if action=="get":
        return {"path": inp.get("path"), "value": _get_path(obj, inp.get("path"))}
