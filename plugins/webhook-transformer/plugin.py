...
def handle_event(event_type, data, cfg):
    ...
    mapped=_map(data if isinstance(data,dict) else {"data":data}, cfg)
    if as_bool(cfg.get("send_notice"), True):
        notice(str(mapped.get("title") or event_type), str(mapped.get("body") or json.dumps(mapped, ensure_ascii=False)[:500]), "info")
    et=str(cfg.get("emit_event") or "").strip()
    if et: publish_event(et, mapped)
    log("info","webhook-transformer","mapped", mapped if isinstance(mapped,dict) else {})
def _get(obj, path):
    cur=obj
...
def _map(data, cfg):
    out={}
    for line in str(cfg.get("mappings") or "").splitlines():
        line=line.strip()
        if not line or line.startswith("#") or "=" not in line: continue
        dst, src = line.split("=",1)
        out[dst.strip()] = _get(data, src.strip())
    if not out: out={"payload": data}
    return out
def handle_action(action, inp, cfg, host):
    if action!="preview": raise ValueError("unknown action")
    sample=inp.get("payload") or inp
    if isinstance(sample,str):
        try: sample=json.loads(sample)
        except Exception: sample={"text":sample}
    return {"mapped": _map(sample, cfg)}
