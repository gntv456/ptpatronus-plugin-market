...
def handle_action(action, inp, cfg, host):
    tools=["site.list","site.stats","search.torrents","downloader.list","subscription.list"]
    if action=="tools":
        allow=split_multi(cfg.get("allow_tools"))
        return {"enabled": as_bool(cfg.get("enabled"), False), "tools": tools, "allow_tools": allow, "note": "MCP JSON-RPC 由宿主 /api/v1/mcp 提供"}
    if action=="status":
        return {"enabled": as_bool(cfg.get("enabled"), False), "has_token": bool(str(cfg.get("token") or "").strip()), "allow_tools": split_multi(cfg.get("allow_tools"))}
