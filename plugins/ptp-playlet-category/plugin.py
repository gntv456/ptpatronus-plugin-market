import shutil
# -*- coding: utf-8 -*-
import json, os, re, time, threading, shutil, socket, ipaddress, hashlib, base64, subprocess, urllib.request, urllib.error, urllib.parse, hmac, struct
...
_status={"running":False,"done":True}
_lock=threading.Lock()
def handle_event(event_type, data, cfg):
    return
def _episode_count(folder: Path):
    exts={".mp4",".mkv",".ts",".avi",".mov",".flv",".m4v"}
    n=0
    for f in folder.rglob("*"):
        if f.is_file() and f.suffix.lower() in exts: n+=1
    return n
def _worker(cfg):
    global _status
    scanned=moved=errors=0
    roots=split_multi(cfg.get("scan_paths")); target=Path(str(cfg.get("target_root") or "").strip() or ".")
    min_ep=as_int(cfg.get("min_episodes"),20) or 20
    dry=as_bool(cfg.get("dry_run"), False)
    try:
        ...
        for root in roots:
            ...
            for d in p.iterdir():
                ...
                try:
                    n=_episode_count(d)
                    if n < min_ep: continue
                    dest=target / d.name
                    if dry:
                        moved+=1; continue
                    if dest.exists():
                        continue
                    shutil.move(str(d), str(dest)); moved+=1
                except Exception:
                    errors+=1
        notice("短剧分类完成", f"扫描 {scanned}, 移动 {moved}, 错误 {errors}", "info" if errors==0 else "warning")
    finally:
        with _lock:
            _status={"running":False,"done":True,"scanned":scanned,"moved":moved,"errors":errors,"finished_at":datetime.now().isoformat(timespec="seconds")}
def handle_action(action, inp, cfg, host):
    ...
    if action=="scan":
        with _lock:
            ...
            _status={"running":True,"done":False,"scanned":0,"moved":0,"errors":0,"started_at":datetime.now().isoformat(timespec="seconds")}
